...
class GCPFunctionsBackend:
    ...
    def _create_handler_zip(self):
        ...
        try:
            with zipfile.ZipFile(ZIP_LOCATION, 'w') as pywren_zip:
                current_location = os.path.dirname(os.path.abspath(__file__))
                module_location = os.path.dirname(os.path.abspath(pywren_ibm_cloud.__file__))
                main_file = os.path.join(current_location, 'entry_point.py')
                pywren_zip.write(main_file, 'main.py', zipfile.ZIP_DEFLATED)
                req_file = os.path.join(current_location, 'requirements.txt')
                pywren_zip.write(req_file, 'requirements.txt', zipfile.ZIP_DEFLATED)
    ...
    def _create_function(self, runtime_name, memory, code, timeout=60, trigger='HTTP'):
        logger.debug("Creating function {} - Memory: {} Timeout: {} Trigger: {}".format(runtime_name, memory, timeout, trigger))
    ...
    def invoke(self, runtime_name, runtime_memory, payload={}):
        ...
        try:
            # Publish message
            fut = self.publisher_client.publish(topic_location, bytes(json.dumps(payload).encode('utf-8')))
